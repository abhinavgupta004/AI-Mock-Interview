...
export default function ScoreRing({ score, size = 96 }) {
  ...
  const color = score >= 80 ? '#00c47a' : score >= 60 ? '#f59e0b' : '#ef4444';

  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#F0F1F5" strokeWidth="7" />
      <circle cx={cx} cy={cy} r={r} fill="none"
        stroke={color} strokeWidth="7" strokeLinecap="round"
        strokeDasharray={circ} strokeDashoffset={circ * (1 - score / 100)}
        transform={`rotate(-90 ${cx} ${cy})`} className="sfill" />
      ...
      <text x={cx} y={cy + 12} dominantBaseline="middle" textAnchor="middle"
        fill="#9ca3af" fontSize="8" fontFamily="Inter, sans-serif">/100</text>
    </svg>
  );
}
